  ...
  return (
    <>
      ...
      >
        ...
        {cart.length > 0 && (
          <span className="absolute -top-2 -right-2 h-5 w-5 rounded-full bg-red-500 text-white text-[0.65rem] font-bold flex items-center justify-center shadow-lg">
            {cart.length}
          </span>
        )}
      </button>
      ...
      >
        ...
        >
          ...
          <ul className="p-4 overflow-y-auto scrollbar-hide h-[calc(100%-140px)] flex flex-col gap-12">
            {cart.map((product) => (
                <li key={product.id} className="flex flex-col gap-1 px-6">
                    <button className="self-end text-sm cursor-pointer" onClick={() => removeFromCart(product.id)}>X</button>
          ...
          <footer className="absolute bottom-0 w-full h-25 p-4">
            <button className="w-full h-full bg-black text-white rounded-xs cursor-pointer hover:bg-gray-800 transition">
                Fechar pedido
            </button>
          </footer>
        </div>
